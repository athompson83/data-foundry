...
CREATE INDEX IF NOT EXISTS rights_cells_source_idx
    ON rights_cells (source_id)
    WHERE source_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS rights_terms_cells_source_idx
    ON rights_terms_cells (source_id)
    WHERE source_id IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS rights_decision_activation_events_decision_idx
    ON rights_decision_activation_events (decision_id);

CREATE INDEX IF NOT EXISTS rights_terms_activation_events_version_idx
