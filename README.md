...
The latest independent provider reconciliation, **2026-09-18**, records the hosted database at migrations `0001`–`0033` with the release's own verification passing read-only, six cache-disabled `verify-full` Hyperdrives (one per runtime role), both R2 buckets, the five dedicated private-canary queues beside the untouched ordinary 14-day pair, and **seven private-canary Workers** whose deployed bundles byte-match the tracked manifests at `55804842f5c5131640dd0435c7d203a66e95b63c` — route-less according to those manifests and their read-back bindings, while custom domains, `workers.dev` and preview-URL flags themselves were not readable with the tooling available to that session (`pnpm cloudflare:readback:check`, added later on 2026-09-18, reads them and queue retention/backlog from the REST API, but has not yet been run against the account); one private-canary cycle passed with six `READY` probes and exactly-once metering ([record](docs/evidence/ua002-hosted-execution-reconciliation-20260918.md)). **No ordinary Data Foundry Worker exists in the account, and neither session created or changed any route, hostname, Cron trigger or public DNS record**; the canonical hostnames still resolve to the historical Vercel placement. The 2026-09-02 refresh that observed the zone as active/full under the standard usage model with zero Data Foundry Workers, Hyperdrives or R2 buckets is retained as superseded history. These are point-in-time provider observations, not deployment authorization for anything public.

The ordinary target topology contains six Workers: edge, public web, usage consumer, acquisition worker, ingestion worker, and MCP worker. The temporary private-canary gate is different: it builds **seven route-less private-canary Worker artifacts** consisting of **six reduced target Workers** plus one no-database harness. CI distinguishes the **six ordinary Worker templates and the seven route-less private-canary templates**.
...
Repository migrations `0027` through `0033`, previously pending hosted application, are **applied on the hosted database** as of 2026-09-18 — the ledger holds `0001`–`0033` with every checksum matching release `2063ea8d72247a9b2643e1c690e37ab55ab14252`, and the 57 historical search-path warnings are closed by `0027`. Do not replay already-ledgered migrations; a later migration must be reviewed and applied through the same direct-TLS operator. Migration `0034` (the direct-customer request allowance, [ADR-0014](docs/decisions/ADR-0014-request-allowance-hard-stop.md)) is in the repository and **pending hosted application** as of 2026-09-19; it raises the expected runtime grant inventory from 286 to 297. A separate synthetic ingestion phase adds one further build profile with its own Queue/DLQ and artifact bucket; it does not widen the thirteen core artifacts or the receipt-only canary, and it has not been deployed.

**Repository state alone designates no Worker release candidate.** Historical revisions are provenance only. Any candidate must contain the reviewed runtime changes, thirteen-artifact gate, tests, and aligned documentation, then pass exact-head verification. **Any later commit, including documentation-only, creates a new release SHA and requires fresh exact-SHA checks** before designation or provider action.
...
The accepted [revenue delivery plan](docs/superpowers/plans/2026-09-08-recurring-revenue-platform.md) targets the HVAC Equipment Specifications & Evidence API on RapidAPI. See [source review](docs/sources/hvac-source-review-20260908.md), [offer and buyer trials](docs/product/hvac-equipment-offer.md), [operator commands](docs/operations.md), and the [preserved platform reference](docs/reference/platform-reference-20260903.md). The new ingestion core supports bounded JSON/CSV; PDF/HTML and larger source partitions need separate runtime qualification. A separate synthetic ingestion Queue/DLQ and artifact bucket must be proved before real-source deployment; the capability canary is not that proof.
