...
| C-05 | **Neither is "the truth".** DOE disclaims compliance determination; EPA disclaims warranty | **Once CCMS reaches `ACTIVE`** (§8 — it is not even approved), publish both with attribution and let the disagreement be visible (rule 3). `APPROVED` is not the gate here: it authorises bounded acquisition into quarantine and nothing further, so a CCMS row may exist locally while remaining unpublishable and unusable as corroboration. Publication waits on validation of what was actually acquired |
