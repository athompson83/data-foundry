...
Other measured limits that shape the design: message body ≤ 128 KB; batch ≤ 100
messages; `max_retries` defaults to 3; retention up to 14 days; **5,000 messages
per second per queue**, above which `send()` throws `Too Many Requests`. No
ordering guarantee is documented, and none is needed — usage events are
...
Runs **after** the response is handed back, inside `ctx.waitUntil`, so the
customer's latency contains no part of metering. `apps/edge/src/index.ts`
currently takes `(request, env)`; it gains `ctx: ExecutionContext`, which is the
only signature change the Worker needs.
