...
import {
  createRuntimeExtractionRegistry,
  type ExtractedRecord,
  type ExtractionProviderRegistry,
  type ExtractionSchema,
} from '@data-foundry/extraction/runtime';
import {
...
import type { FixtureBinding } from './fixtures.js';
...
export interface PipelineOptions {
  ...
  readonly promoteBeforePublished?: boolean;
...
const buildSnapshotCandidates = (
  ...
  processingRuntimeDigest?: string,
  ...
  return streams
    ...
    .map((stream): SnapshotCandidate => {
      const mappingDigest = sha256Hex(stableStringify({
        ...
        ...(processingRuntimeDigest === undefined ? {} : { processingRuntimeDigest }),
...
export class ArtifactPipeline {
  ...
  constructor(options: PipelineOptions) {
    ...
    this.#extraction = options.extraction ?? createRuntimeExtractionRegistry();
   ...
   */
  async runArtifacts(sourceKey: string, input: ArtifactPipelineInput): Promise<SourceRunResult> {
    return this.runSource(sourceKey, input);
  }

  async runSource(sourceKey: string, input?: ArtifactPipelineInput): Promise<SourceRunResult> {
    ...
    try {
      ...
      if (input === undefined) await this.#requireInternalRights(source, entry, 'APPROVED_OR_ACTIVE', [
      ...
      let acquisition: { outcome: string; diagnostics: readonly string[]; fetchedAt: string };
      const acquired: AcquiredArtifact[] = [];
      if (input !== undefined) {
        acquisition = { outcome: 'FETCHED', diagnostics: [], fetchedAt: input.fetchedAt };
        acquired.push(...input.artifacts);
      } else {
      ...
      for (const [index, artifact] of fetched.artifacts.entries()) {
        const stored = fetched.stored[index];
      ...
      for (const { artifact, body } of acquired) {
        for (const stream of plan.streams) {
          ...
          const records = await this.#extraction.extract({ artifact, body,
            ...(this.#options.maxRecords === undefined ? {} : { maxRecords: this.#options.maxRecords - extracted.length }),
          }, schema);
          if (extracted.length + records.length > (this.#options.maxRecords ?? 100_000)) {
            throw new PipelineConfigurationError('INGESTION_RECORD_LIMIT');
          }
      ...
      if (this.#options.promoteBeforePublished === true) await this.promoteCanonicalView();
  ...
  async #resolveBatch(
    ...
    const snapshotCandidates = buildSnapshotCandidates(
      ...
      snapshotArtifacts,
      snapshotObservedAt,
      this.#options.processingRuntimeDigest,
    );
    ...
    return this.store.driver.transaction(async (tx) => {
      ...
      for (const candidate of snapshotCandidates) {
        const [latest] = await tx.query<{
          ...
          processing_runtime_digest: string | null;
          processing_generation: number;
          candidate_is_newer: boolean;
          candidate_is_replay: boolean;
        }>(
          `SELECT id, observed_at, snapshot_digest, artifact_set_digest, processing_runtime_digest, processing_generation,
            ...
            ORDER BY observed_at DESC, processing_generation DESC, snapshot_digest COLLATE "C" DESC
            LIMIT 1`,
          [source.id, candidate.stream, candidate.observedAt, candidate.snapshotDigest],
        );
        const runtimeReplay = latest !== undefined && this.#options.processingRuntimeDigest !== undefined &&
          latest.processing_runtime_digest !== this.#options.processingRuntimeDigest &&
          new Date(latest.observed_at).getTime() === Date.parse(candidate.observedAt) &&
          latest.artifact_set_digest === candidate.artifactSetDigest;
        if (latest !== undefined && !latest.candidate_is_newer && !runtimeReplay) {
          diagnostics.push(
        ...
        const [acceptance] = await tx.query<{ id: string }>(
          `INSERT INTO source_stream_snapshot_acceptances
             (source_id, source_stream, observed_at, snapshot_digest,
              ...
              processing_runtime_digest, processing_generation)
          ...
          [
            ...
            this.#options.processingRuntimeDigest ?? null,
            runtimeReplay ? Number(latest!.processing_generation) + 1 :
              latest !== undefined && new Date(latest.observed_at).getTime() === Date.parse(candidate.observedAt)
                ? Number(latest.processing_generation) : 0,
          ],
        );
  ...
  async #storedRights(source: Source): ReturnType<typeof loadStoredRightsContext> {
    ...
    const loaded = loadStoredRightsContext(this.store.driver, source.id, this.#rightsAsOf ?? this.#options.now);
    ...
    const decision = evaluateRights(
      {
        ...
        channel: 'INTERNAL_PROCESSING',
        asOf: this.#rightsAsOf ?? this.#options.now,
        conditionReceipts: [],
      },
      context.snapshot,
    );
    if (decision.permitted) this.#admittedIntents.set(stableStringify([source.id, sourceStatusRequirement, intent]),
      { source, entry, requirement: sourceStatusRequirement, intent });
    return decision;
  }

  /** Final publication guard: reload every scope used for canonical writes at the database clock. */
  async assertCurrentInternalRights(asOf: string): Promise<void> {
    this.#rightsContexts.clear();
    this.#rightsAsOf = asOf;
    for (const { source, entry, requirement, intent } of [...this.#admittedIntents.values()]) {
      await this.#requireInternalRights(source, entry, requirement, [intent]);
    }
  }
  ...
  async #ensureVertical(): Promise<Vertical> {
    ...
    if (this.#options.persistedGovernanceOnly === true) {
      this.#vertical = await this.store.getVerticalBySlug(this.config.slug);
      if (this.#vertical === null) throw new PipelineConfigurationError('INGESTION_VERTICAL_MISSING');
  ...
  async #ensureSource(entry: SourceRegistryEntry, verticalId: VerticalId): Promise<Source> {
    ...
    let source: Source;
    if (this.#options.persistedGovernanceOnly === true) {
      const rows = await this.store.driver.query('SELECT * FROM sources WHERE vertical_id = $1 AND domain = $2 AND source_type = $3', [verticalId, entry.domain, entry.source_type]);
      if (rows[0] === undefined) throw new PipelineConfigurationError('INGESTION_SOURCE_MISSING');
      source = mapSource(rows[0]);
    } else {
      source = await this.store.upsertSource(toSourceInsert(entry, verticalId));
    }
