...
describe('Supabase connector migration packet export', () => {
  ...
  it('requires the independently pinned contiguous 0001 through 0034 repository chain', () => {
    expect(() =>
      build({ migrations: migrations.filter(({ version }) => version !== '0002') }),
    ).toThrow(/expected 34 contiguous migrations.*missing.*0002/i);
  });
  it('uses the private-schema transform checksum as the application ledger authority', () => {
    ...
    expect(plan.repositoryMigrationCount).toBe(34);
    expect(plan.pendingMigrationCount).toBe(34);
  ...
  it('exports only 0027 through 0034 for the exact hosted 0001 through 0026 ledger prefix, and only 0034 for the 2026-09-18 hosted ledger', () => {
    ...
    expect(upgradePlan.pendingMigrationCount).toBe(8);
    expect(upgradePlan.packets.map(({ version }) => version)).toEqual(['0027', '0028', '0029', '0030', '0031', '0032', '0033', '0034']);

    // The hosted database holds 0001–0033 as of 2026-09-18; the next catch-up is exactly one packet.
    const hostedPlan = build({
      appliedMigrations: fullPlan.packets.slice(0, 33).map(({ version, filename, checksum }) => ({ version, filename, checksum })),
    });
    expect(hostedPlan.appliedMigrationCount).toBe(33);
    expect(hostedPlan.pendingMigrationCount).toBe(1);
    expect(hostedPlan.packets.map(({ version }) => version)).toEqual(['0034']);
  });
  ...
  it('executes the bootstrap and each packet atomically against an isolated PostgreSQL database', async () => {
    ...
    try {
      ...
      expect(rows.at(-1)?.version).toBe('0034');
  ...
  it('loads packet bytes from the immutable Git object after source identity verification', async () => {
    ...
    try {
      const releaseSha = await createGitSourceFixture(repository, 34);
      ...
      const plan = await buildSupabaseMigrationPlanFromGit(
        { releaseSha, repositoryRoot: repository, appliedMigrations: [] },
        {
          verifySourceIdentity: async (sha, root) => {
            ...
            await writeFile(
              join(repository, 'db/migrations/0034_fixture.sql'),
      ...
      expect(plan.packets.at(-1)?.transformedSql).toContain("SELECT 'committed-0034'");
