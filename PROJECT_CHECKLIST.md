...
| --- | --- | --- | --- | --- | --- | --- | --- |
| FOUNDATION-001 | Foundation | Canonical schema, migrations, provenance, query model, acquisition, extraction, normalization, resolution, and storage architecture | DONE | Synthetic HVAC sources run end to end with evidence-backed canonical output and deterministic checks | `tests/e2e/factory-proof.test.ts`; `README.md`; migrations through `0022` in the integration candidate; driver-managed resolution transaction with executor-affinity, failure-rollback, unresolved-record rollback, explicit source-record revision states, and real-Postgres reconciliation-concurrency regression | None | 2026-08-23 |
