...
| FOUNDATION-006 | Foundation | Canonical Cloudflare and database topology | BLOCKED | Account, zone/routes, Hyperdrive, production Postgres, and bindings are configured and evidenced | `apps/edge/wrangler.toml`; `apps/mcp-worker/wrangler.toml`; `apps/usage-consumer/wrangler.toml`; `apps/web/wrangler.toml`; `tooling/scripts/check-cloudflare-topology.ts`; `tooling/scripts/check-cloudflare-artifacts.ts`; `docs/owner-actions/cloudflare-deployment.md` | Repository topology, credential-free Wrangler dry-runs, and PGlite-leak checks are implemented; account/zone/Hyperdrive/provider credentials and live proof remain owner-controlled | — |
...
| ID | Phase | Description | Status | Acceptance criteria | Evidence | Dependencies / blocker | Completed |
| --- | --- | --- | --- | --- | --- | --- | --- |
| MVP-001 | MVP | Rights-grant model supports different lawful uses and access tiers | IN_PROGRESS | Accepted rights architecture and implementation are integrated into the release candidate without implying source clearance | Accepted ADR-0010; `packages/rights-engine`; migrations `0014_rights_grant_matrix.sql` and `0016_core_rights_hardening.sql`; exact-target DERIVE checks cover direct output evidence plus recursive dependencies; public/search rights are rebound from one explicit instant per request; `sources:readiness` evaluates the canonical seven surface bundles at an explicit instant from live database evidence or a schema/digest-validated qualified snapshot and returns `UNKNOWN` without evidence; surface, lineage, readiness, migration, and PostgreSQL 16 negative controls | Candidate integration/merge remains; source-specific legal/rights review is independently required | — |
...
| MVP-010 | MVP | Deployable MCP / agent-access surface | IN_PROGRESS | The six generic canonical-query tools run through MCP 2026-07-28 Streamable HTTP with exact one-vertical `MCP/NONE` auth, live rights freshness, privacy-safe Queue analytics, generated runtime parity, and no local-only runtime in the Cloudflare bundle | `apps/mcp`; `apps/mcp-worker`; `packages/access-auth`; `db/migrations/0018_mcp_access_channel.sql`; `verticals/hvac/mcp.yaml`; MCP Worker and Cloudflare artifact tests | Candidate integration/merge; MCP hostname/route, Hyperdrive/Queue bindings, exact source grants, customer packaging, and live-client proof | — |
