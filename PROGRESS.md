...
## Current session — 2026-09-16 UA-002 connector exception and first-source decision sheet
...
- `UA-001` advanced on the unblocked side. A fillable [rights decision sheet](docs/owner-actions/ua-001-doe-ccms-rights-decision.md) now states the minimum that earns revenue: six operation/channel cells, `ACQUIRE`/`STORE`/`CACHE`/`NORMALIZE` on `INTERNAL_PROCESSING` plus `SERVE_API_ACCESS` and `SELL_API_ACCESS` on `DIRECT_CUSTOMER_API`, with every other surface left `UNKNOWN` so it refuses by default.
