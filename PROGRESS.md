...
## Current session — 2026-09-16 no first-source candidate is selectable
...
- **DOE CCMS** is not selectable on access rather than rights: the edge returns **403 to an honest agent for `robots.txt` itself**, so crawl permission cannot be established without impersonating a browser, and all 112 data.gov sitemap shards (559,455 URLs) confirm no catalogue entry.
